...
const log: IDebugger = debug('app:FindCasosPorFaixaEtariaUseCase');

export class FindCasosPorFaixaEtariaUseCase implements UseCase {
  ...
  execute(params: RequestFilter): Promise<Casos[]> {
    return this.service.findCasosPorFaixaEtaria(params);
  }
}