...
const log: debug.IDebugger = debug('app:FindTotalCasosPorSituacaoController');

export class FindTotalCasosPorSituacaoController extends ControllerOperation {
  ...
  async handle(req: Request, res: Response): Promise<Response<void, Record<string, any>>> {
    try {
      ...
      log(error);
      console.error(error);
      return res.status(500).json({ error: 'erro ao buscar total de casos por situação.' });
    }
  }
}