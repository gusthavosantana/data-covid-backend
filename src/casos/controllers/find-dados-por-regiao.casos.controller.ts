...
const log: debug.IDebugger = debug('app:FindDadosPorRegiaoController');

export class FindDadosPorRegiaoController extends ControllerOperation {
  ...
  async handle(req: Request, res: Response): Promise<Response<void, Record<string, any>>> {
    try {
      ...
      log(error);
      console.error(error);
      return res.status(500).json({ error: 'erro ao buscar dados de casos por região.' });
    }
  }
}